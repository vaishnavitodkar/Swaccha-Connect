...
import { OfficerDashboard } from "@/app/components/dashboard/officer-dashboard";
...
export default async function DashboardPage() {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("id,full_name,avatar_url,role,municipality_id,ward_id")
    .eq("id", user.id)
    .single();
  ...
  if (profile?.role === "officer" && profile.municipality_id) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center px-5 py-20">
      <section className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/50 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
        ...
        <p className="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-400">The officer workspace is available to officer accounts assigned to a municipality. An admin workspace will be available here soon.</p>
